...
import streamlit_app as initial_page
import numpy as np
import pandas as pd
...
if Ratio == "Yes":
    Path = st.file_uploader("Upload your Coil Path. The file must contain only numeric coordinates." \
    " Do not include headers, titles, or text in any column. ", type=["txt", "csv", "xlsx"])
    if Path is not None: 
        type_of_file = Path.name.split(".")[-1]
        if type_of_file == "xlsx":
            df = pd.read_excel(Path, delimiter=" " or "," or ";")
        
        elif type_of_file in ("csv","txt"):
            df =pd.read_csv(Path, delimiter=" " or "," or ";")
        ...
        if current and point != "":
            ...
            st.write("Magnetic field for the coil object:", coil.biotSavart1p(r0=(np.array(point)) , I=current, integration_method=method))
...
if Ratio == "No":
        ...
        if shape is "Solenoid":
            ...
            if n_turns and Pa and Pb and radius != "":
                n_turns = int(n_turns)
                Pa = np.fromstring(Pa, sep=",", dtype=float)
                Pb = np.fromstring(Pb, sep=",", dtype=float)
                radius = float(radius)
                if length == "":
                    length = 0.1
                length = float(length)
